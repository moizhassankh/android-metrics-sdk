// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: proto/event/event.proto

package ai.benshi.proto.event;

public interface LogRequestOrBuilder extends
    // @@protoc_insertion_point(interface_extends:event.LogRequest)
    com.google.protobuf.MessageOrBuilder {

  /**
   * <pre>
   * Optional.  If not set, set by API servers.
   * If not set, API server uses the value set on each log record.
   * </pre>
   *
   * <code>uint64 platform_id = 1;</code>
   * @return The platformId.
   */
  long getPlatformId();

  /**
   * <pre>
   * Optional.  Must be set on each child log record or here.
   * </pre>
   *
   * <code>.common.UserInfo user_info = 2;</code>
   * @return Whether the userInfo field is set.
   */
  boolean hasUserInfo();
  /**
   * <pre>
   * Optional.  Must be set on each child log record or here.
   * </pre>
   *
   * <code>.common.UserInfo user_info = 2;</code>
   * @return The userInfo.
   */
  ai.benshi.proto.common.UserInfo getUserInfo();
  /**
   * <pre>
   * Optional.  Must be set on each child log record or here.
   * </pre>
   *
   * <code>.common.UserInfo user_info = 2;</code>
   */
  ai.benshi.proto.common.UserInfoOrBuilder getUserInfoOrBuilder();

  /**
   * <pre>
   * Optional.  If not set, set by API servers.
   * If not set, API server uses the value set on each log record.
   * </pre>
   *
   * <code>.common.Timing timing = 3;</code>
   * @return Whether the timing field is set.
   */
  boolean hasTiming();
  /**
   * <pre>
   * Optional.  If not set, set by API servers.
   * If not set, API server uses the value set on each log record.
   * </pre>
   *
   * <code>.common.Timing timing = 3;</code>
   * @return The timing.
   */
  ai.benshi.proto.common.Timing getTiming();
  /**
   * <pre>
   * Optional.  If not set, set by API servers.
   * If not set, API server uses the value set on each log record.
   * </pre>
   *
   * <code>.common.Timing timing = 3;</code>
   */
  ai.benshi.proto.common.TimingOrBuilder getTimingOrBuilder();

  /**
   * <pre>
   * Optional.
   * </pre>
   *
   * <code>.common.ClientInfo client_info = 4;</code>
   * @return Whether the clientInfo field is set.
   */
  boolean hasClientInfo();
  /**
   * <pre>
   * Optional.
   * </pre>
   *
   * <code>.common.ClientInfo client_info = 4;</code>
   * @return The clientInfo.
   */
  ai.benshi.proto.common.ClientInfo getClientInfo();
  /**
   * <pre>
   * Optional.
   * </pre>
   *
   * <code>.common.ClientInfo client_info = 4;</code>
   */
  ai.benshi.proto.common.ClientInfoOrBuilder getClientInfoOrBuilder();

  /**
   * <pre>
   * Optional. Information about the user device.
   * </pre>
   *
   * <code>.common.Device device = 24;</code>
   * @return Whether the device field is set.
   */
  boolean hasDevice();
  /**
   * <pre>
   * Optional. Information about the user device.
   * </pre>
   *
   * <code>.common.Device device = 24;</code>
   * @return The device.
   */
  ai.benshi.proto.common.Device getDevice();
  /**
   * <pre>
   * Optional. Information about the user device.
   * </pre>
   *
   * <code>.common.Device device = 24;</code>
   */
  ai.benshi.proto.common.DeviceOrBuilder getDeviceOrBuilder();

  /**
   * <code>repeated .event.User user = 7;</code>
   */
  java.util.List<User>
      getUserList();
  /**
   * <code>repeated .event.User user = 7;</code>
   */
  User getUser(int index);
  /**
   * <code>repeated .event.User user = 7;</code>
   */
  int getUserCount();
  /**
   * <code>repeated .event.User user = 7;</code>
   */
  java.util.List<? extends UserOrBuilder>
      getUserOrBuilderList();
  /**
   * <code>repeated .event.User user = 7;</code>
   */
  UserOrBuilder getUserOrBuilder(
      int index);

  /**
   * <code>repeated .event.CohortMembership cohort_membership = 8;</code>
   */
  java.util.List<CohortMembership>
      getCohortMembershipList();
  /**
   * <code>repeated .event.CohortMembership cohort_membership = 8;</code>
   */
  CohortMembership getCohortMembership(int index);
  /**
   * <code>repeated .event.CohortMembership cohort_membership = 8;</code>
   */
  int getCohortMembershipCount();
  /**
   * <code>repeated .event.CohortMembership cohort_membership = 8;</code>
   */
  java.util.List<? extends CohortMembershipOrBuilder>
      getCohortMembershipOrBuilderList();
  /**
   * <code>repeated .event.CohortMembership cohort_membership = 8;</code>
   */
  CohortMembershipOrBuilder getCohortMembershipOrBuilder(
      int index);

  /**
   * <code>repeated .event.View view = 11;</code>
   */
  java.util.List<View>
      getViewList();
  /**
   * <code>repeated .event.View view = 11;</code>
   */
  View getView(int index);
  /**
   * <code>repeated .event.View view = 11;</code>
   */
  int getViewCount();
  /**
   * <code>repeated .event.View view = 11;</code>
   */
  java.util.List<? extends ViewOrBuilder>
      getViewOrBuilderList();
  /**
   * <code>repeated .event.View view = 11;</code>
   */
  ViewOrBuilder getViewOrBuilder(
      int index);

  /**
   * <code>repeated .event.AutoView auto_view = 25;</code>
   */
  java.util.List<AutoView>
      getAutoViewList();
  /**
   * <code>repeated .event.AutoView auto_view = 25;</code>
   */
  AutoView getAutoView(int index);
  /**
   * <code>repeated .event.AutoView auto_view = 25;</code>
   */
  int getAutoViewCount();
  /**
   * <code>repeated .event.AutoView auto_view = 25;</code>
   */
  java.util.List<? extends AutoViewOrBuilder>
      getAutoViewOrBuilderList();
  /**
   * <code>repeated .event.AutoView auto_view = 25;</code>
   */
  AutoViewOrBuilder getAutoViewOrBuilder(
      int index);

  /**
   * <code>repeated .delivery.Request request = 12;</code>
   */
  java.util.List<ai.benshi.proto.delivery.Request>
      getRequestList();
  /**
   * <code>repeated .delivery.Request request = 12;</code>
   */
  ai.benshi.proto.delivery.Request getRequest(int index);
  /**
   * <code>repeated .delivery.Request request = 12;</code>
   */
  int getRequestCount();
  /**
   * <code>repeated .delivery.Request request = 12;</code>
   */
  java.util.List<? extends ai.benshi.proto.delivery.RequestOrBuilder>
      getRequestOrBuilderList();
  /**
   * <code>repeated .delivery.Request request = 12;</code>
   */
  ai.benshi.proto.delivery.RequestOrBuilder getRequestOrBuilder(
      int index);

  /**
   * <code>repeated .delivery.Insertion insertion = 13;</code>
   */
  java.util.List<ai.benshi.proto.delivery.Insertion>
      getInsertionList();
  /**
   * <code>repeated .delivery.Insertion insertion = 13;</code>
   */
  ai.benshi.proto.delivery.Insertion getInsertion(int index);
  /**
   * <code>repeated .delivery.Insertion insertion = 13;</code>
   */
  int getInsertionCount();
  /**
   * <code>repeated .delivery.Insertion insertion = 13;</code>
   */
  java.util.List<? extends ai.benshi.proto.delivery.InsertionOrBuilder>
      getInsertionOrBuilderList();
  /**
   * <code>repeated .delivery.Insertion insertion = 13;</code>
   */
  ai.benshi.proto.delivery.InsertionOrBuilder getInsertionOrBuilder(
      int index);

  /**
   * <code>repeated .event.Impression impression = 14;</code>
   */
  java.util.List<Impression>
      getImpressionList();
  /**
   * <code>repeated .event.Impression impression = 14;</code>
   */
  Impression getImpression(int index);
  /**
   * <code>repeated .event.Impression impression = 14;</code>
   */
  int getImpressionCount();
  /**
   * <code>repeated .event.Impression impression = 14;</code>
   */
  java.util.List<? extends ImpressionOrBuilder>
      getImpressionOrBuilderList();
  /**
   * <code>repeated .event.Impression impression = 14;</code>
   */
  ImpressionOrBuilder getImpressionOrBuilder(
      int index);

  /**
   * <code>repeated .event.Action action = 15;</code>
   */
  java.util.List<Action>
      getActionList();
  /**
   * <code>repeated .event.Action action = 15;</code>
   */
  Action getAction(int index);
  /**
   * <code>repeated .event.Action action = 15;</code>
   */
  int getActionCount();
  /**
   * <code>repeated .event.Action action = 15;</code>
   */
  java.util.List<? extends ActionOrBuilder>
      getActionOrBuilderList();
  /**
   * <code>repeated .event.Action action = 15;</code>
   */
  ActionOrBuilder getActionOrBuilder(
      int index);

  /**
   * <code>repeated .event.Diagnostics diagnostics = 23;</code>
   */
  java.util.List<Diagnostics>
      getDiagnosticsList();
  /**
   * <code>repeated .event.Diagnostics diagnostics = 23;</code>
   */
  Diagnostics getDiagnostics(int index);
  /**
   * <code>repeated .event.Diagnostics diagnostics = 23;</code>
   */
  int getDiagnosticsCount();
  /**
   * <code>repeated .event.Diagnostics diagnostics = 23;</code>
   */
  java.util.List<? extends DiagnosticsOrBuilder>
      getDiagnosticsOrBuilderList();
  /**
   * <code>repeated .event.Diagnostics diagnostics = 23;</code>
   */
  DiagnosticsOrBuilder getDiagnosticsOrBuilder(
      int index);
}
